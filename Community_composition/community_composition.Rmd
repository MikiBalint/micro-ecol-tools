...
# remove doubleton OTUs. Singletons were removed during the sequence cleanup from the dataset.
Abund.95.NoDouble = Abund.95[,which(vegan::specnumber(Abund.95,MARGIN=2)>2)]

patho.TotCount = apply(Abund.95.NoDouble,2,sum)
IsAbund = patho.TotCount > 2*mean(patho.TotCount)
patho.some = Abund.95.NoDouble[,IsAbund]
...
The code takes about `r round(patho.anova.time[3]/60)` minutes with `r NBoot` bootstraps. It shows that most variation is explained by biome, and C_N ratios.
